...
Shopify is a SaaS platform, which means that the hosting is fully managed by Shopify. Shopify itself is based on Ruby on Rails and hosted at Google's GCP. The APIs are distributed globally using Cloudflare's Edge Network.
